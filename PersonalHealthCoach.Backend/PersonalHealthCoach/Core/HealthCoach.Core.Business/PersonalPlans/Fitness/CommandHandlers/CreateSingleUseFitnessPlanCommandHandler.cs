...
internal sealed class CreateSingleUseFitnessPlanCommandHandler : IRequestHandler<CreateSingleUseFitnessPlanCommand, Result<FitnessPlan>>
{
    private readonly IRepository repository;
    private readonly IEfQueryProvider queryProvider;
    private readonly IHttpClient httpClient;

    public CreateSingleUseFitnessPlanCommandHandler(IRepository repository, IEfQueryProvider queryProvider, IHttpClientFactory httpClientFactory)
    {
    ...
    public async Task<Result<FitnessPlan>> Handle(CreateSingleUseFitnessPlanCommand request, CancellationToken cancellationToken)
    {
        ...
        return await Result.FirstFailureOrSuccess(userResult, dataResult)
            ...
            .Bind(async command =>
                await httpClient.Post<RequestFitnessPlanCommand, RequestFitnessPlanCommandResponse>(command))
            .Bind(response =>
            {
                ...
                return FitnessPlan.Create(request.UserId,
                    new List<Workout> { workout1, workout2, workout3, workout4, workout5, workout6, workout7 });
            });
    }
}