...
class TipGenerator:
    ...
    def generate_tips(self):
        ...
        tips.append(generator.generate_daily_calories_burned_tip())
        tips.append(generator.generate_interesting_tip())
    ...
    def generate_daily_calories_burned_tip(self):
        if 'Progress' in self.profile_data:
            progress = self.profile_data['Progress']
        else:
            raise KeyError("Progress key doesn't exist.")
        ...
        if 'ExerciseLogs' in progress[-1]:
            ...
            workout = self.profile_data['Progress'][-1]['ExerciseLogs']
            minutes = round(calculate_minutes_per_workout(workout))
            calories_burned_by_workout = calculate_calories_per_workout(minutes,MET,weight) + 50
        else:
            calories_burned_by_workout = 0
        
        calories_burned_by_steps = generator.calculate_calories_burned_by_steps_today()

        total_calories_burned = calories_burned_by_steps + calories_burned_by_workout
        if total_calories_burned == 0:
            tip = "It looks like you haven't done any physical activity today. No problem, the day is long and you have all the time you need. Consistency is the key in living a healthy life and achieving your personal goal."
        else:
            tip = "You managed to burn " + str(total_calories_burned) + " calories today from walking and exercising. These two activities together bring you the greatest benefits and if you practice them constantly you will see the changes in your life."
        generated_tip = {
    ...
    def generate_interesting_tip(self):
        interesting_tips = self.tips_data['Interesting']
        tip = random.choice(interesting_tips)
        generated_tip = {
                            "Type": "Interesting",
                            "Importance Level": "Medium",
                            "Tip": tip
